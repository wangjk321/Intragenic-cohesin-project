setwd("/home/wang/LocalTest/Mlearning")
mt <- read.csv("All161b14c.matrix",sep="\t",stringsAsFactors = F,
               colClasses = c("character",rep("numeric",161),rep("character",14)))
rownames(mt)<-mt[,1]
mt <- mt [,-1]
mt[mt=="."] <- NA
mt <- na.omit(mt)
...
#write.table(mtout,file="bino161_normalizedContinuous14.matrix",
